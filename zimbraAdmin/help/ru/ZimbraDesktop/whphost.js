	...
	if(gnChecked==0||isSamePath(gaProj[gnChecked].sPPath,aRProj[0].sPPath))
	{
		if(gnChecked!=0)
		{
			if(aRProj[0].sLangId!=gaProj[0].sLangId)
				alert("В объединенной справочной системе "+aRProj[0].sPPath+" используется другой язык по сравнению с основной справочной системой, поэтому функции указателя и полнотекстового поиска в объединенной системе будут отключены.");
		}
		...
		alert("Неуспешная загрузка, нажмите 'Обновить'.");
}
