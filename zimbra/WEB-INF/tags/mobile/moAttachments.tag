        ...
        <div>
            <span>
                <div>
                    <div>
                        <span>
                            <c:if test="${mailbox.features.briefcases && ua.isiPad eq false}"><input type="checkbox" name="attachIds" value="${fn:escapeXml(partId)}"/></c:if>
                            <c:choose>
                                <c:when test="${fn:containsIgnoreCase(part.contentType, 'officedocument.wordprocessingml.document')}">
                                    <mo:img src="zimbra/ImgMSWordDoc.png" alt="${fn:escapeXml(part.displayName)}"/>
                                </c:when>
                                <c:when test="${fn:containsIgnoreCase(part.contentType, 'officedocument.spreadsheetml.sheet')}">
                                    <mo:img src="zimbra/ImgMSExcelDoc.png" alt="${fn:escapeXml(part.displayName)}"/>
                                </c:when>
                                <c:when test="${fn:containsIgnoreCase(part.contentType, 'officedocument.presentationml.presentation')}">
                                    <mo:img src="zimbra/ImgMSPowerpointDoc.png" alt="${fn:escapeXml(part.displayName)}"/>
                                </c:when>
                                <c:otherwise>
